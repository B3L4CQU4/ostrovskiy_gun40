...
namespace CasinoGame.Games {
    public class DiceGame : CasinoGameBase {
        ...
        protected override void FactoryMethod() {
            // резервируем структуру т.к. в т.з. CasinoGameBase
            // 5.	У класса должен быть защищённый метод FactoryMethod, который вызывается в конструкторе и является абстрактным.
            // базовый конструктор выполняется до инициализации полей наследника 
            // получается что фабрика отрабатывает до того, как присваиваются значения
